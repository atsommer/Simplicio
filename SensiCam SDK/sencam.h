...
int WINAPI WRITE_REGISTRY_VALUES(void);
...
//@ver3.21
void WINAPI SETTIMEOUT(int timeout1,int timeout2);
void WINAPI GETTIMEOUT(int *timeout1,int *timeout2);
...
//@ver3.21
int WINAPI TESTCOC(int *mode, int *trig,
             int *roixmin, int *roixmax,
             int *roiymin, int *roiymax,
             int *hbin, int *vbin,
             char *tab,int *tablength);
...
int WINAPI SET_POWERDOWN(int pdtime, int pdwakeup);
...
int WINAPI DMA_IMAGE_START(int board,int width, int height,unsigned short *frame,HANDLE picevent);
...
int WINAPI SET_DICAM_WAIT(int time);
...
int WINAPI GET_CAMERA_ID(int board,int *id);
...
int WINAPI GET_CAMERA_CCD(int board,int *ccdtyp);
...
int WINAPI GET_CAMERA_TYP(int board,int *camtyp);
...
int WINAPI GET_CAMERA_STAT(int board,int select,int *stat);
...
int WINAPI SET_CAMERA_VAL(int board,int select,int val);
...
int WINAPI BEGIN_WAIT_IMAGE(int boardnr,HANDLE event);
//this function is only avaiable for WINDOWS NT
//WIN95/98 returns with error -13
...
int WINAPI END_WAIT_IMAGE(int boardnr);
//this function is only avaiable for WINDOWS NT
//WIN95/98 returns with error -13
//disables board function to create events, when


int WINAPI GET_COC_SETTING(int *mode, int *trig,
                           int *roixmin, int *roixmax,
                           int *roiymin, int *roiymax,
                           int *hbin, int *vbin,
                           char *tab,int len);
...
int WINAPI CHECK_BOARD_MEMORY(int board, int len);
